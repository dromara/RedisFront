...
public class ConnectServiceImpl implements ConnectService {
    ...
    public List<ConnectInfo> getConnectListByName(String name) {
        ...
        return DerbyUtils.getInstance().querySql(sql.toUpperCase()).stream().map(this::mapToConnectInfo).toList();
    }

    @Override
    public List<ConnectInfo> getAllConnectList() {
        String sql = "select * from rf_connect";
        List<Map<String, Object>> result = DerbyUtils.getInstance().querySql(sql.toUpperCase());
    ...
    public ConnectInfo getConnect(Object id) {
        ...
        return DerbyUtils.getInstance().querySql(sql.toUpperCase()).stream().map(this::mapToConnectInfo).findAny().orElse(null);
    }

    @Override
    public void update(ConnectInfo connectInfo) {
        var sql = this.buildUpdateSql(connectInfo);
        DerbyUtils.getInstance().exec(sql.toUpperCase());
    }
    ...
    public void initDatabase() {
        ...
        DerbyUtils.getInstance().exec(sqlData.toUpperCase());
    }
